...
import "./HackathonTestimonial.css"; // 👈 include your CSS
...
export default function HackathonTestimonial() {
  return (
    <div className="pb-10 pt-10">
      {/* Heading */}
      <div className="text-center font-semibold tracking-wide mb-8">
        <p className="text-3xl sm:text-4xl lg:text-5xl">Circle Members Are</p>
        <p className="text-[#FA3E67] text-3xl sm:text-4xl lg:text-5xl">
          Winning!
        </p>
      </div>
      ...
      <div className="wrapper">
        {testimonials.map((t, index) => (
          <div key={t.id} className={`item item${index + 1}`}>
            <div className="bg-[#1D1D1D] text-start text-white rounded-2xl border border-[#707070] p-6 w-[300px] h-[190px] shadow-xl flex flex-col justify-between">
              {/* Text */}
              <p className="text-[13px] leading-snug line-clamp-5">{t.text}</p>

              {/* Profile */}
              <div className="flex items-center gap-2 mt-3">
                <div className="w-8 h-8 rounded-full overflow-hidden border border-[#4D4D4D] flex-shrink-0">
                  <img
                    src={t.img}
                    alt={t.name}
                ...
                <div className="leading-relaxed">
                  <h3 className="font-semibold text-xs">{t.name}</h3>
                  <div className="flex items-center gap-2 text-[#FA3E67]">
                    <p className="text-[10px]">{t.role}</p>
